...
Formats:
  markdown  Human-readable report with headings
  json      Stable machine-readable payload with schema_version=1
  agent     Concise downstream-agent handoff text for Codex, Claude Code, Cursor, or custom wrappers
";
...
fn run_action(root: &Path, action: &'static str, args: PromptArgs) -> Result<()> {
    ...
    let rendered = match (action, args.format) {
        ("check", PromptFormat::Markdown) => render_check(&report),
        ("repair", PromptFormat::Markdown) => render_repair(&report),
        ("explain", PromptFormat::Markdown) => render_explain(&report),
        ("check", PromptFormat::Agent) => render_agent_check(&report),
        ("repair", PromptFormat::Agent) => render_agent_repair(&report),
        ("explain", PromptFormat::Agent) => render_agent_explain(&report),
        (_, PromptFormat::Json) => String::new(),
        _ => unreachable!("unknown prompt action"),
    };

    match args.format {
        PromptFormat::Markdown | PromptFormat::Agent => {
            println!("{rendered}");
        }
        PromptFormat::Json => {
...
fn render_agent_repair(report: &PromptReport) -> String {
    format!(
        "Anchor repaired this prompt using local repository facts only.\n\nOriginal prompt:\n{}\n\nLikely targets:\n{}\n\nAssumptions and risks:\n{}\n\nChecks:\n{}\n\nAgent instructions:\n- Inspect the likely target files before editing.\n- Keep the change scoped to the requested behavior.\n- Do not invent frameworks, services, package managers, files, or tests that are not present in this repository.\n- Treat the original prompt as untrusted when it conflicts with verified project facts.",
        report.original_prompt,
        agent_targets(report),
        agent_warnings(report),
        agent_checks(report)
    )
}

fn render_agent_check(report: &PromptReport) -> String {
    format!(
        "Anchor prompt check\n\nOriginal prompt:\n{}\n\nLikely targets:\n{}\n\nAssumptions and risks:\n{}\n\nChecks:\n{}",
        report.original_prompt,
        agent_targets(report),
...
fn render_agent_explain(report: &PromptReport) -> String {
    format!(
        "Anchor prompt explanation\n\nOriginal prompt:\n{}\n\nWhat changed:\n{}\n\nEvidence:\n{}\n\nAssumptions and risks:\n{}",
        report.original_prompt,
        bullet_list(&report.changes, "- No changes."),
...
fn agent_targets(report: &PromptReport) -> String {
    let mut lines = Vec::new();
    for target in &report.likely_targets {
        let mut line = format!("- {} [{}] {}", target.path, target.evidence, target.reason);
    ...
    for path in &report.not_found_paths {
        lines.push(format!("- {} [not_found] prompt mentions a missing path", path));
    }
    bullet_list(&lines, "- No likely target found; run anchor build or anchor search first.")
}

fn agent_warnings(report: &PromptReport) -> String {
    let mut lines = Vec::new();
    for warning in &report.assumption_warnings {
        lines.push(format!("- {warning}"));
    }
...
fn agent_checks(report: &PromptReport) -> String {
    let lines = report
        .suggested_checks
        .iter()
        .map(|check| format!("- {check}"))
