    ...
    def walk(value: Any) -> None:
        if isinstance(value, dict):
            ...
            if kind == "tool_use" and isinstance(name, str):
                record(name)
            elif kind in {"function_call", "tool_call"} and isinstance(name, str):
                record(name)
...
''',
    ...
    container_name = f"anchor-codex-verify-{os.getpid()}-{int(time.time() * 1000)}"
...
def run_mode(args: argparse.Namespace, task: dict[str, Any], mode: str, out_root: Path) -> dict[str, Any]:
    ...
    tools = parse_tool_counts(run_dir / "codex.jsonl")
    result = {
        "mode": mode,
        "agent": agent,
        "git": metrics,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run baseline vs Anchor Codex on a DeepSWE task.")
    ...
    summary = {
        "schema": "anchor.native_deepswe_codex_pair.v1",
