...
fn cli_gate_require_receipts_flags_raw_write_but_not_anchor_edit() {
    ...
    .unwrap();
    assert!(Command::new("git").arg("init").arg("-q").current_dir(dir.path()).status().unwrap().success());
    Command::new(anchor).arg("--root").arg(dir.path()).arg("build").output().unwrap();
    ...
    std::fs::write(dir.path().join("src/a.py"), "def f():\n    return 1\n").unwrap();
    Command::new(anchor).arg("--root").arg(dir.path()).arg("context").arg("f").output().unwrap();
    Command::new(anchor)
        .arg("--root").arg(dir.path())
        .arg("edit").arg("src/a.py")
        .arg("--action").arg("replace")
        .arg("--pattern").arg("return 1")
        .arg("--content").arg("return 2")
        .output().unwrap();
